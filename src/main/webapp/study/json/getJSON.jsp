...
<script type="text/javascript">
	...
	$('button').eq(0).on('click', function() {
		$.getJSON("/study/json/dept.json", function(data) {
			...
			console.log("detpno = " + data.deptno);
	...
	$('button').eq(1).on('click', function() {
		$.getJSON("/study/json/depts.json", function(data) {
			...
			for (var i=0; i<data.length; i++){
				console.log("detpno = " + data[i].deptno);
...
<script id="temp1" type="text/xxx-mytemplate">
	<li class="list-group list-group-item-info">{{deptno}}, {{dname}}, {{loc}}</li>
</script>

<script id="temp2" type="text/xxx-mytemplate">
	{{#each .}}
	<li class="list-group list-group-item-warning">{{deptno}}, {{dname}}, {{loc}}</li>
	{{/each}}
</script>
